...
export default function Users() {
  ...
  const kpiActive = list.filter((u) => u.status === 'ACTIVE').length;
  const kpiInactive = list.filter((u) => u.status === 'SUSPENDED' || u.status === 'INACTIVE').length;
  ...
  return (
    <>
      <Topbar title="Người dùng" subtitle="Thứ Ba, 18/08/2026 · users" onSearch={setQ} searchPlaceholder="Tìm tên, email, SĐT…" />
      <div className="content">
        <section className="kpi-row">
          <div className="kpi c1"><div className="top"><span className="ic"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg></span></div><div className="num">{kpiTotal}</div><div className="lbl">Tổng tài khoản USER</div></div>
          ...
          <div className="kpi c3"><div className="top"><span className="ic"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 9v4M12 17h.01M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z" /></svg></span></div><div className="num">{kpiInactive}</div><div className="lbl">SUSPENDED / INACTIVE</div></div>
          <div className="kpi c4"><div className="top"><span className="ic"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 5v14M5 12h14" /></svg></span></div><div className="num">{rows.length}</div><div className="lbl">Đang hiển thị</div></div>
        </section>
        ...
        <div className="panel">
          <div className="panel-head"><div><h3>Danh sách người dùng</h3><div className="sub">accounts (role USER) · {rows.length} bản ghi</div></div></div>
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th>Người dùng</th><th>Email</th><th>SĐT</th><th>Giới tính / Tuổi</th><th>Ngày tạo TK</th><th>Trạng thái</th>
              ...
              <tbody>
                {rows.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td className="mono">{u.createdAt ? String(u.createdAt).slice(0, 10) : '—'}</td>
                    <td>
                      <span className={`tag ${u.status === 'ACTIVE' ? 'ok' : u.status === 'SUSPENDED' ? 'danger' : 'neutral'}`}>{u.status}</span>
                    </td>
                    <td>
          ...
          <div className="table-footer">
            <span>Hiển thị <strong>{rows.length}</strong> trên tổng <strong>{kpiTotal}</strong></span>
          </div>
        </div>
      </div>
        ...
        {detail && (
          <>
            ...
            <div className="detail-row">
              ...
              <span className="val">
                <span className={`tag ${detail.status === 'ACTIVE' ? 'ok' : detail.status === 'SUSPENDED' ? 'danger' : 'neutral'}`}>{detail.status}</span>
              </span>
            </div>
          </>
        )}
