...
export default function Vaccines() {
  ...
  const kpiCatCount = new Set(
    list.map((v) => v.cat).filter((c) => c !== "" && c != null),
  ).size;
  ...
  const save = async (e) => {
    ...
    const doses =
      form.doses === "" || form.doses == null ? 1 : Number(form.doses);
  ...
  return (
    <>
      <Topbar
        title="Vắc xin & phác đồ"
        subtitle="Thứ Ba, 18/08/2026 · vaccines "
        onSearch={setQ}
      />
      <div className="content">
        <section className="kpi-row">
            ...
            <div className="top">
              <span className="ic">
                <svg
                  ...
                  strokeWidth="2"
                >
                  <path d="M11 2 3 10l3 3 8-8-3-3Z" />
                </svg>
              </span>
            </div>
            <div className="num">{kpiTotal}</div>
            <div className="lbl">Loại vắc xin</div>
          </div>
          <div className="kpi c2">
            ...
            <div className="num">{kpiActive}</div>
          ...
          <div className="kpi c3">
            ...
            <div className="num">{list.filter((v) => v.proto).length}</div>
            <div className="lbl">Có phác đồ</div>
          </div>
          <div className="kpi c4">
            ...
            <div className="num">{kpiCatCount}</div>
            <div className="lbl">Nhóm danh mục</div>
          </div>
        </section>
        ...
        <div className="panel">
          <div className="panel-head">
            <div>
              ...
              <div className="sub">
                vaccines · vaccination_protocols · {rows.length} bản ghi
              </div>
            </div>
          </div>
              ...
              <tbody>
                {rows.map((v) => (
                  <tr key={v.id}>
                    ...
                    <td className="mono">
                      {v.doses}
                      {v.interval ? ` · ≥${v.interval}d` : ""}
                    </td>
                    <td>
                      <span className="proto-chip">{v.proto}</span>
                    </td>
                    <td>
                      <span className="tag ok">{v.status}</span>
                    </td>
                    <td>
                      <div className="row-actions">
                        <button
                          className="row-btn outline"
                          type="button"
                          onClick={() => setDetail(v)}
                        >
                          Chi tiết
        ...
        {detail && (
          <>
            <div className="detail-row">
              ...
              <span className="val">{CATS[detail.cat]}</span>
            </div>
            <div className="detail-row">
              <span className="lbl">Phòng bệnh</span>
              <span className="val">{detail.disease}</span>
            </div>
            <div className="detail-row">
              <span className="lbl">Số mũi / Khoảng cách</span>
              <span className="val">
                {detail.doses} mũi
                {detail.interval ? ` · ≥${detail.interval} ngày` : ""}
              </span>
            </div>
            <div className="detail-row">
              <span className="lbl">Phác đồ</span>
              <span className="val">{detail.proto}</span>
            </div>
            <div className="detail-row">
              <span className="lbl">Trạng thái</span>
              <span className="val">
                <span className="tag ok">{detail.status}</span>
              </span>
            </div>
          </>
        )}
      ...
      >
        <div className="field">
          <label>
            Tên ngắn <span className="req">*</span>
          </label>
          <input
            value={form.name}
        ...
        <div className="field-row">
          ...
          <div className="field">
            ...
            <select
              value={
                form.cat === "" || form.cat == null ? "" : String(form.cat)
              }
        ...
        <div className="field">
          ...
          <input
            ...
            placeholder="Mô tả phác đồ"
          />
        </div>

        <div className="field">
          <label>Ảnh vắc xin (URL)</label>
          {form.imageUrl ? (
            <img
