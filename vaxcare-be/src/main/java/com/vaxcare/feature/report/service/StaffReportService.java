...
public class StaffReportService {
    ...
    public StaffReportResponse getReport(
        ...
        for (Object[] row : appointmentRepository.countGroupByStatus(
                scope.facilityId(), fromDate, toDate)) {
        ...
        BigDecimal revenue = appointmentRepository.sumCompletedRevenue(
                scope.facilityId(), fromDate, toDate);
        if (revenue == null) {
        ...
        List<FacilityStatItem> facilityStats = buildFacilityStats(
                scope.facilityId(), fromDate, toDate);
    ...
    public byte[] exportAppointmentsCsv(
        ...
        sb.append("appointment_id,date,time,status,patient,phone,")
                .append("vaccine,price,qr_code,facility,staff,")
                .append("ai_recommended,note\n");

        for (Appointment a : list) {
            String patient = a.getUser() != null
                    ? nullToEmpty(a.getUser().getFullName())
                    : "";
            ...
            String vaccine = a.getVaccine() != null
                    ? nullToEmpty(a.getVaccine().getVaccineName())
                    : "";

            String facility = a.getFacility() != null
                    ? nullToEmpty(a.getFacility().getFacilityName())
                    : "";

            String staff = a.getStaff() != null
                    ? nullToEmpty(a.getStaff().getFullName())
                    : "";

            String price = a.getPrice() != null
                    ? a.getPrice().toPlainString()
                    : "";

            String time = a.getTimeSlot() != null
                    ? a.getTimeSlot().toString()
                    : "";

            String date = a.getAppointmentDate() != null
            ...
            sb.append(csv(a.getAppointmentId())).append(',')
                    .append(csv(date)).append(',')
                    .append(csv(time)).append(',')
                    .append(csv(status)).append(',')
                    .append(csv(patient)).append(',')
                    .append(csv(phone)).append(',')
                    .append(csv(vaccine)).append(',')
                    .append(csv(price)).append(',')
                    .append(csv(nullToEmpty(a.getQrCode()))).append(',')
                    .append(csv(facility)).append(',')
                    .append(csv(staff)).append(',')
                    .append(csv(ai)).append(',')
                    .append(csv(nullToEmpty(a.getNote())))
                    .append('\n');
        }
    ...
    /**
     * Export summary (KPI + daily + ranking + reaction mix) as CSV.
     */
    @Transactional(readOnly = true)
    public byte[] exportSummaryCsv(
        ...
        StaffReportResponse report = getReport(
                currentAccountId, facilityId, days, fromDateParam, toDateParam);
        ...
        appendMeta(sb, "fromDate", report.getFromDate());
        appendMeta(sb, "toDate", report.getToDate());
        appendMeta(sb, "days", report.getDays());
        appendMeta(sb, "facilityId", report.getFacilityId());
        appendMeta(sb, "facilityName", report.getFacilityName());

        // KPI
        StaffReportKpi k = report.getKpi();
        if (k != null) {
            appendKpi(sb, "appointments", k.getAppointments());
            appendKpi(sb, "completed", k.getCompleted());
            appendKpi(sb, "cancelled", k.getCancelled());
            appendKpi(sb, "checkedIn", k.getCheckedIn());
            appendKpi(sb, "pending", k.getPending());
            appendKpi(sb, "confirmed", k.getConfirmed());
            appendKpi(sb, "completionRate", k.getCompletionRate());
        }

        // Daily series
        sb.append("\ndaily_date,label,count\n");
        if (report.getDailySeries() != null) {
            for (DailyCountItem d : report.getDailySeries()) {
                sb.append(csv(d.getDate())).append(',')
                        .append(csv(d.getLabel())).append(',')
                        .append(d.getCount())
                        .append('\n');
            }
        }
        ...
        // Vaccine ranking
        sb.append("\nrank,vaccine,shots,pct\n");
        if (report.getVaccineRanking() != null) {
            for (VaccineRankItem v : report.getVaccineRanking()) {
                sb.append(v.getRank()).append(',')
                        .append(csv(v.getVaccineName())).append(',')
                        .append(v.getShots()).append(',')
                        .append(v.getPct())
                        .append('\n');
            }
        }
        ...
        // Reaction mix
        sb.append("\nseverity,label,count,pct\n");
        if (report.getReactionMix() != null) {
            for (ReactionSeverityItem r : report.getReactionMix()) {
                sb.append(csv(r.getSeverity())).append(',')
                        .append(csv(r.getLabel())).append(',')
        ...
        if (account.getRole() == Role.MEDICAL_STAFF) {
            ...
            if (staff == null || staff.getFacility() == null) {
                throw new BadRequestException(
                        "Tài khoản nhân viên y tế này chưa được gán cơ sở tiêm chủng!");
            }
            return new ResolvedScope(
                    staff.getFacility().getFacilityId(),
                    staff.getFacility().getFacilityName());
        }
    ...
    private List<DailyCountItem> buildDailySeries(
            List<Object[]> rows,
            LocalDate from,
            LocalDate to) {
    ...
    private List<FacilityStatItem> buildFacilityStats(
            Long facilityId,
            LocalDate fromDate,
            LocalDate toDate) {

        List<Object[]> rows = appointmentRepository.countGroupByFacilityStatus(
                facilityId, fromDate, toDate);

        Map<Long, FacilityStatItem> stats = new LinkedHashMap<>();
        ...
        for (Object[] row : rows) {
            ...
            FacilityStatItem item = stats.computeIfAbsent(id, k ->
                    FacilityStatItem.builder()
                            .facilityId(id)
                            .facilityName(name)
                            .appointments(0)
                            .completed(0)
            ...
            item.setAppointments(item.getAppointments() + count);

            if (status == AppointmentStatus.COMPLETED) {
                item.setCompleted(item.getCompleted() + count);
            }
            if (status == AppointmentStatus.PENDING) {
                item.setPending(item.getPending() + count);
            }
        }

        // Revenue per facility
        List<Appointment> appointments = appointmentRepository.findInDateRange(
                facilityId, fromDate, toDate);

        Map<Long, BigDecimal> revenueMap = new HashMap<>();
        for (Appointment appointment : appointments) {
            if (appointment.getStatus() == AppointmentStatus.COMPLETED
                    && appointment.getFacility() != null
                    && appointment.getPrice() != null) {

                Long id = appointment.getFacility().getFacilityId();
                revenueMap.merge(id, appointment.getPrice(), BigDecimal::add);
            }
        }

        for (FacilityStatItem item : stats.values()) {
            long total = item.getAppointments();
            ...
            item.setRevenue(revenueMap.getOrDefault(item.getFacilityId(), BigDecimal.ZERO));
        }
    ...
    private void appendMeta(StringBuilder sb, String key, Object value) {
        sb.append(csv("meta")).append(',')
                .append(csv(key)).append(',')
                .append(csv(value))
                .append('\n');
    }

    private void appendKpi(StringBuilder sb, String key, Object value) {
        sb.append(csv("kpi")).append(',')
                .append(csv(key)).append(',')
                .append(value)
                .append('\n');
    }
